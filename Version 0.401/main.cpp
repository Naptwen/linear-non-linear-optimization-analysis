...
int main()
{
    ...
    while(true)
    {
        ...
        struct tm tm;
        localtime_s(&tm, &t);
        while (tm.tm_hour > 6 && tm.tm_min > 29)
        {
            setcolor(7, 0);
            ...
            for (int k = 0; k < 7; k++)
            {
                ...
                gotoxy(12, 2*k + 1);
                printf("%.2f", cur_stock[k]);
                past_stock[k] = cur_stock[k];

                setcolor(3, 0);
                gotoxy(0, 2 * k + 2);
                printf("[max] %.2f", cur_max[k]);
                gotoxy(20, 2 * k + 2);
                printf("[min] %.2f", cur_min[k]);
                gotoxy(40, 2 * k + 2);
                printf("[gap] %.2f", cur_max[k] - cur_min[k]);

                if (tm.tm_hour >= 16)
                {
                    gotoxy(60, 2 * k + 2);
                    printf("[end] max %.2f", cur_max[k]);
                    gotoxy(80, 2 * k + 2);
                    printf("[end] min %.2f\n", cur_min[k]); 
                    gotoxy(100, 2 * k + 2);
                    printf("[gap] %.2f\n", cur_max[k] - cur_min[k]);
