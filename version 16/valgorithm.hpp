...
vector<float> mnz(float* begin, float* end){
    vector<float> ANS;
    float max = *max_element(begin, end + 1);
    while (begin <= end)
    {
...
vector<float> mmnz(float* begin, float* end){
    vector<float> ANS;
    float max = *max_element(begin, end + 1);
    float min = *min_element(begin, end + 1);
    while (begin <= end)
    {
        ANS.push_back((begin[0] - min)/(max - min));
        begin = (begin + 1);
    }
    return ANS;
}
...
vector<float> stdz(float* begin, float* end){
    vector<float> ANS;
    float mu = μ(begin, end + 1);
    float v = σ2(begin, end + 1);
    if(v == 0){
        perror("STD is 0!");
        return ANS;
    }
...
void html_2xy_plot(const string name, ofstream *html_file, int w, int h,  const vector<float> &data_1, const vector<float> &data_2){
    if(!data_1.empty() && !data_2.empty()){
        html_canvas_create(html_file, w, h, name);
        *html_file << "<script>\n";
        *html_file << "var pen = canvas.getContext(\"2d\");\n";
        int sz = min(data_1.size(), data_2.size());
        vector<float> val_x = mmnz((float*)&data_1[0], (float*)&data_1[sz-1]);
        vector<float> val_y = mmnz((float*)&data_2[0], (float*)&data_2[sz-1]);
        float x_padding = 25;
        float y_padding = 25;
        int px = 15;
        *html_file << "pen.lineWidth = 1;\n";
        ...
        *html_file << "pen.stroke();\n";
        ...
        float _h_ = 0;
        for(int i = 0; i < sz; i ++){
            _w_ = (w - x_padding * 2) * val_x[i] + x_padding;
...
//required html_canvas_create
void v2_html_2xy_plot(  ofstream *html_file, 
                        int w, int h, 
                        const string color, const float point_sz,
                        const vector<float> &data_1, const vector<float> &data_2){
    if(!data_1.empty() && !data_2.empty()){
        *html_file << "<script>\n";
        *html_file << "var pen = canvas.getContext(\"2d\");\n";
        int sz = min(data_1.size(), data_2.size());
        vector<float> val_x = mmnz((float*)&data_1[0], (float*)&data_1[sz-1]);
        vector<float> val_y = mmnz((float*)&data_2[0], (float*)&data_2[sz-1]);
        float x_padding = w * 0.01f;
        float y_padding = h * 0.01f;
        *html_file << "pen.lineWidth = 1;\n";
        *html_file << "pen.moveTo("<< x_padding << "," << 0 <<")\n";
        *html_file << "pen.lineTo("<< x_padding << "," << h <<")\n";
        *html_file << "pen.moveTo("<< 0 << "," << h - y_padding <<")\n";
        ...
        *html_file << "pen.stroke();\n";
        float _w_ = 0;
        float _h_ = 0;
        *html_file << "pen.fillStyle = \"" + color + "\";\n";
        *html_file << "pen.lineWidth = 0;\n";
        *html_file << "pen.transform(1, 0, 0, -1, 0," << h << ");\n";
        for(int i = 0; i < sz; i ++){
            _w_ = (w - x_padding * 4) * val_x[i] + x_padding;
            _h_ = (h - y_padding * 4) * val_y[i] + y_padding;
            *html_file << "pen.beginPath();\n";
            *html_file << "pen.arc("<< _w_ << "," 
                                    << _h_
                                    << ", "
...
//required html_canvas_create
void v2_html_2xy_line(  ofstream *html_file, 
                        int w, int h, 
                        const string color, const float line_sz,
                        const vector<float> data_1, const vector<float> data_2){
    if(!data_1.empty() && !data_2.empty()){
        *html_file << "<script>\n";
        *html_file << "var pen = canvas.getContext(\"2d\");\n";
        ...
        float x_padding = w * 0.01f;
        float y_padding = h * 0.01f;
        *html_file << "pen.lineWidth = "<< line_sz << ";\n";
        *html_file << "pen.moveTo("<< x_padding << "," << 0 <<")\n";
        *html_file << "pen.lineTo("<< x_padding << "," << h <<")\n";
        *html_file << "pen.moveTo("<< 0 << "," << h - y_padding <<")\n";
        *html_file << "pen.lineTo("<< w << "," << h - y_padding <<")\n";
        ...
        *html_file << "pen.transform(1, 0, 0, -1, 0," << h << ");\n";
        *html_file << "pen.beginPath();\n";
        *html_file << "pen.strokeStyle =\"" + color + "\";\n";
        for(int i = 0; i < sz; i++){
            _w_ = (w - x_padding * 4) * val_x[i] + x_padding;
            _h_ = (h - y_padding * 4) * val_y[i] + y_padding;
            *html_file << "pen.lineTo("<< _w_ << "," << _h_ <<")\n";
