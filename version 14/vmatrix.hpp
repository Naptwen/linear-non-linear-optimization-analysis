...
template<typename K>
void show_2d_vector(vector<vector<K>> A){
    for(auto vec : A){
        for(auto v : vec){
            printf("%.8f,  ", v);
        }
...
template<typename K>
vector<vector<K>> zero_vector(vector<vector<K>> A){
    #pragma omp for
    for(int i = 0; i < A.size(); i++)
...
template<typename K>
vector<vector<K>> REDUCE(vector<vector<K>> A, bool show = true){
    int k = 0;
    if(show){
        printf("\tORIGIN\n");
        show_2d_vector(A);
...
template<typename K>
vector<vector<K>> GELIMINATION(vector<vector<K>>A, bool show = false){
    A = REDUCE(A, show);
    int i = 0;
    while(i < A.size()){
        #pragma omp for
        for(int j = 0; j < A[i].size(); j++)
            if(A[i][j] != 0){
                ...
                for(int k = j + 1; k < A[i].size(); k++){
                    const char sign = (SIGN(A[i][k])) * (SIGN(p));
...
template<typename K>
vector<vector<K>> ROWECHELONFORM(vector<vector<K>>A, int sz, bool show = false){
    vector<vector<K>> R = GELIMINATION(A, show);
    #pragma omp for
...
vector<K> MATRIX_SOLUTION(vector<vector<K>> mat, vector<K> var, bool show = false){
    ...
    if(var.size() == mat.size()){
        ...
        vector<vector<K>> tmp = ADDCOL<K>(mat, var, mat[0].size());
        vector<vector<K>> ans = ROWECHELONFORM<float>(tmp, tmp[0].size()-1, show);
        //find equation variables
        vector<K> temp;
        vector<K> x(mat[0].size(), 0); 
...
vector<vector<K>> MATRIX_INVERSE(vector<vector<K>> A, bool show = false){
    ...
    vector<vector<K>> INVA = ROWECHELONFORM(B, A.size(), show);
    if(show){
        printf("\tECHELON\n");
        show_2d_vector(INVA);
    }
...
class MATRIX{
    public:
        ...
        vector<int> ROWS_INDPENDENT(){
            MATRIX AT = TRANS();
            vector<vector<K>> RAT = ROWECHELONFORM(AT.mat, AT.mat[0].size());
        ...
        MATRIX INVERSE(){
            if(mat.size() == mat[0].size()){
                ...
                vector<vector<K>> AINVER = ROWECHELONFORM(A__.mat, A__.m); 
        ...
        // }
        int RANK(){
            vector<vector<K>> reduce = GELIMINATION(mat);
