...
public:
    ...
    void population_setting(float* population1, int p1size, float* population2, int p2size){
        ...
        pm1 = μ(population1, population1 + (p1size-1));
        ps1 = sqrt(σ2(population1, population1 + (p1size-1), POPUL));
        N1 = p1size;
        if(population2 != NULL){
            pm2 = μ(population2, population2 + (p2size-1));
            ps2 = sqrt(σ2(population2, population2 + (p2size-1), POPUL));
            N2 = p2size;
        }
    }
    void sample_setting(float* sample1, int s1size, float* sample2, int s2size){
        if(sample1 == NULL) perror("NULL SAMPLE 1 IS INPUT!");
        m1 = μ(sample1, sample1 + (s1size-1));
        v1 = σ2(sample1, sample1 + (s1size-1), SAMPLE);
        n1 = s1size;
        if(sample1 != NULL){
            m2 = μ(sample2, sample2 + (s2size-1));
            v2 = σ2(sample2, sample2 + (s2size-1), SAMPLE);
            n2 = s2size;
        }
    }
