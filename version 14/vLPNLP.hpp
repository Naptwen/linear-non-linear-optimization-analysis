...
public:
    ...
    vector<float> THE_LEVENBERG_MARQUARDT_GAUSS_NEWTON_AMIJO_LINE_SEARCH(vector<float> x, bool record = false)
    {
        // 0 < ηa < ηe < 1
        float ηe = 0.7f; 
        float ηa = 0.2f; 
    ...
    vector<float> QUASI_BFGS_NEWTON_AMIJO_LINE_SEARCH(vector<float> x, bool record = false)
    {
        float rc = 0.5f;
        float ηa = 0.4f;
        float δ = 0.5f;
